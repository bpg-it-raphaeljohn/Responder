        ...
        try:
            ...
            if data[11:12] == b"\x01":
                ...
                SSLsock = ssl.wrap_socket(
                    self.request, certfile=cert, keyfile=key, ssl_version=ssl.PROTOCOL_TLS_SERVER, server_side=True
                )
            ...
            if data[len(data) - 4 :] == b"\x03\x00\x00\x00":
                ...
                SSLsock = ssl.wrap_socket(
                    self.request, certfile=cert, keyfile=key, ssl_version=ssl.PROTOCOL_TLS, server_side=True
                )
                data = SSLsock.read(8092)
                if FindNTLMNegoStep(data) == b"\x01\x00\x00\x00":
                    x = RDPNTLMChallengeAnswer(NTLMSSPNtServerChallenge=NetworkRecvBufferPython2or3(Challenge))
