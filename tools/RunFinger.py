...
parser.add_option(
    "-f", "--filename", action="store", help="Target file", dest="Filename", metavar="ips.txt", default=None
)
parser.add_option(
    "-t",
    "--timeout",
    ...
    print(
        (
            "[SMB2]:['{}', Os:'{}', Build:'{}', Domain:'{}', Bootime: '{}', Signing:'{}', RDP:'{}', SMB1:'{}', MSSQL:'{}']".format(
                host, WindowsVers, str(WindowsBuildVers), Domain, Bootime, signing, RDP, SMB1, SQL
            )
        )
    )
    SaveRunFingerToDb(
...
def OsNameClientVersion(data):
    try:
        ...
        if length <= 255:
            OsVersion, ClientVersion = tuple(
                [e.replace("\x00", "") for e in data[46 + length :].split("\x00\x00\x00")[:2]]
            )
            return OsVersion, ClientVersion
    except:
