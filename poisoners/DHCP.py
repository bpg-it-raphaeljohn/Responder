...
RespondTo = [
    _f for _f in [x.upper().strip() for x in config.get("Responder Core", "RespondTo").strip().split(",")] if _f
]
DontRespondTo = [
    _f for _f in [x.upper().strip() for x in config.get("Responder Core", "DontRespondTo").strip().split(",")] if _f
]
...
class DHCPACK(Packet):
    ...
    def calculate(self, DHCP_DNS):
        ...
        if DHCP_DNS:
            self.fields["Op6Str"] = socket.inet_aton(RespondWithIP()).decode("latin-1") + socket.inet_aton(
                DNSIP2
            ).decode("latin-1")
        else:
...
def ParseDHCPCode(data, ClientIP, DHCP_DNS):
    ...
    # DHCP Inform
        ...
        IP_Header = IPHead(
            SrcIP=socket.inet_aton(ROUTERIP).decode("latin-1"), DstIP=socket.inet_aton(CurrentIP).decode("latin-1")
        )
        Packet = DHCPACK(
            Tid=PTid.decode("latin-1"),
            ClientMac=MacAddr.decode("latin-1"),
...
def SendDiscover():
    ...
    IP_Header = IPHead(
        SrcIP=socket.inet_aton("0.0.0.0").decode("latin-1"), DstIP=socket.inet_aton("255.255.255.255").decode("latin-1")
    )
    Packet = DHCPDiscover()
    Packet.calculate()
